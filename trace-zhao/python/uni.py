...
# plt.xticks(np.linspace(0, num_bins-1, 6), labels=f'{int(x/(num_bins-1)*100)}%' for x in np.linspace(0, num_bins-1, 6))
# ([f'{int(x/(num_bins-1)*100)}%' for x in np.linspace(0, num_bins-1, 6)], fontsize=16)
plt.yticks(range(max_level), labels=[str(i) for i in range(max_level)], fontsize=18)
plt.tight_layout()
plt.savefig("hitmap_150m_uni.svg", format="svg")  # SVG format
plt.show()
